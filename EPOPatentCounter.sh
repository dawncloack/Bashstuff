...
if [[ -n $(echo $1 | grep -oE '[[:alpha:]]') ]] # If $1 is anything but numbers
	...
	declare -ir initialYear=${1:-1900} #if the input input is correct, it is assigned, if it's null it defaults.
fi

if (( $initialYear >= $finalYear )) # If the final year is smaller than the initial one, error.
	then
	UsageMSG
...
declare -ar yearSpace=($(seq --separator=' ' $initialYear $finalYear)) # This declaration makes a read only array with the list of years. 
declare -a valueSpace # This declaration creates an array for the hits in each year of the array. Each year and values per year share the same array index
...
do
	...
	headerSpace[fileCount]=$(cat $nextFile | sed -n '3p' | cut -f1 --delimiter=\;) # The third line of each CSV, containing the number of hits per search and the name of the search, is copied.
	(( fileCount++ ))
...
do
	printf "\"\";\"\";%s\n" "$element" #To make it more pleasant visually the headers are copied in the third column.
done

printf "\"\";\"\";\"%s\"\n" "(total results, filtered by year)"

echo "\"Year\";\"Number of patents\";\"\"" #This is the header of the significant part of the .csv file

readonly writerMax=$(( $(date +"%Y") - $initialYear )) #The number of entries is the current date minus the initial date.
writerCount=0
